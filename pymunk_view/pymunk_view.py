...
class PymunkView(BaseView):
    ...
    def __init__(self, time_on_screen):
        ...
        shape = pymunk.Segment(body, [0, 10], [self.window.width, 10], 0.0)
        shape.friction = 10
        self.space.add(shape, body)
        self.static_lines.append(shape)

        body = pymunk.Body(body_type=pymunk.Body.STATIC)
        shape = pymunk.Segment(body, [self.window.width - 50, 10], [self.window.width, 30], 0.0)
        shape.friction = 10
        self.space.add(shape, body)
        self.static_lines.append(shape)
    ...
    def on_draw(self):
        ...
        self.draw_line_one("Pymunk physics engine integration")
